...
describe('LetterArchivePanel su-rek archive flow', () => {
  it('keeps recommendation letters on the shared TU background payload', () => {
    assert.match(source, /suRek:\s*\{\s*imageBase64:\s*'',\s*fileName:\s*'',\s*mimeType:\s*'image\/png'\s*\}/);
    assert.match(source, /backgrounds\?\.suRek\?\.imageBase64/);
    assert.match(source, /suRek:\s*\{\s*\.\.\.empty\.suRek,\s*\.\.\.sharedBackground\s*\}/);
  });
