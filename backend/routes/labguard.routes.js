...
const LABGUARD_ROLES = ['Admin', 'Laboran', 'Supervisor'];

router.use(verifyRole(LABGUARD_ROLES));
    ...
    return new Promise((resolve, reject) => {
        routerQueue = routerQueue.then(async () => {
            ...
            try {
                ...
                const val = await handler(client);
                resolve(val);
            }
            catch (err) {
                reject(err);
            ...
            // Catch error to allow the next chained promise to execute
        });
    });
}
...
router.get('/config/labs-only-order', (_req, res) => {
...
router.get('/router/status', requireSession, async (_req, res) => {
    if (!HAS_CONFIG) {
        return res.json({
            status: 'simulated',
...
router.get('/interfaces', requireSession, async (_req, res) => {
    if (!HAS_CONFIG) {
...
router.get('/interfaces/traffic', requireSession, async (_req, res) => {
    if (!HAS_CONFIG) {
...
router.get('/router/uplink-traffic', requireSession, async (_req, res) => {
    if (!HAS_CONFIG) {
...
router.get('/site-policies', requireSession, async (_req, res) => {
    if (!HAS_CONFIG) {
...
router.get('/site-policies/address-list/:listName', requireSession, async (req, res) => {
...
router.post('/site-policies/address-list', requireSession, async (req, res) => {
...
router.patch('/site-policies/address-list/:entryId', requireSession, async (req, res) => {
...
router.delete('/site-policies/address-list/:entryId', requireSession, async (req, res) => {
    const entryId = decodeURIComponent(req.params.entryId);
...
router.post('/site-policies/create-list', requireSession, async (req, res) => {
    const listName = String(req.body.listName || '').trim();
...
router.delete('/site-policies/delete-list', requireSession, async (req, res) => {
    const listName = String(req.body?.listName || req.query?.listName || '').trim();
...
router.post('/interfaces/:id/toggle', requireSession, async (req, res) => {
...
router.post('/interfaces/:id/bandwidth', requireSession, async (req, res) => {
    const id = decodeURIComponent(req.params.id);
...
router.get('/router/clients', requireSession, async (_req, res) => {
    if (!HAS_CONFIG) {
        return res.json({
            leases: [
